...
class ReplayBuffer:
    ...
    def add(self, states, action_num, prob_STSP, prob, avail, avail_bi, rewards, next_states, dones):  # 将数据加入buffer
        self.buffer.append((states, action_num, prob_STSP, prob, avail, avail_bi, rewards, next_states, dones))

    def sample(self, _batch_size, device):  # 从buffer中采样数据,数量为batch_size
        transitions = random.sample(self.buffer, _batch_size)
        states, action_num, prob_STSP, prob, avail, avail_bi, rewards, next_states, dones = zip(*transitions)

        states = torch.tensor(states, dtype=torch.float).to(device)
        action_num = torch.tensor(action_num, dtype=torch.int64).view(-1, 1).to(device)
        prob_STSP = torch.tensor(prob_STSP, dtype=torch.float).to(device)
        ...
        return states, action_num, prob_STSP, prob, avail, avail_bi, rewards, next_states, dones
...
class PPO:
    ...
    def take_action(self, state, avail, avail_bi, node, status):
        ...
        return prob_STSP.detach().cpu().numpy(), action, action_num, action_prob.detach().cpu().numpy()

    def update(self, replay_buffer, batch_size):
        device = self.device
        states, action_num, prob_STSP, prob, avail, avail_bi, rewards, next_states, dones = replay_buffer.sample(batch_size, device)
        # 时序差分目标
        states = states.squeeze(1)
        avail = avail.squeeze(1)
        nonavail = 1-avail
        avail_bi = avail_bi.squeeze(1)
        prob_STSP = prob_STSP.squeeze(2)
            ...
            with tqdm(total=int(episodes / 10), desc='Iteration %d' % i) as pbar:
                for i_episode in range(int(episodes / 10)):
                    ...
                    while not status.done:
                        ...
                        prob_STSP, action, action_num, prob = self.take_action(state, avail, avail_bi, node, status)
                        ...
                        if action is None:
                            reward -= 500
                        replay_buffer.add(state, action_num, prob_STSP, prob, avail, avail_bi, reward, next_state, env.done)
                        status = next_status
    ...
    def train_loop(self, buffer_size=2048, minimal_buffer_size=128, batch_size=64, instance_list=None, episodes=500, plot_process=True):
        replay_buffer = ReplayBuffer(buffer_size)
        return_l = []
        makespan_l = []
        instances_makespan = []
                ...
                for i_episode in range(int(episodes / 10)):
                    ...
                    for instance in instance_list:
                        ...
                        while not status.done:
                            ...
                            if action is None:
                                reward -= 500
                            replay_buffer.add(state, action_num, prob_STSP, prob, avail, avail_bi, reward, next_state,
                                              env.done)
                            status = next_status
                            instance_reward += reward
...
if __name__ == '__main__':
    ...
    my_dvl = PPO(hidden_dim=64, state_d=5*5*12, action_d=5*5, actor_lr=5e-4, critic_lr=1e-5)
    instance_names = ['IS{}J{}M{}R{}'.format(3, 5, 5, i+10) for i in range(10)]
    return_list, makespan_list, instance_makespan = my_dvl.train_loop(instance_list=instance_names,episodes=120)
