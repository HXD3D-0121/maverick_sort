"""
Maverick-SORT — Professional Edition v5.0 (HF AI Enhanced)
============================================================
Refactored Pro Tier with Unified Data Upload Hub + Hugging Face AI Integration

New in v5.0:
  - 🤖 Maverick Copilot: global sidebar assistant + dedicated FAQ page
...
with st.sidebar:
    ...
    if st.session_state.copilot_expanded:
        ...
        for i, q in enumerate(quick_qs):
            with q_cols[i % 2]:
                if st.button(q, key=f"cq_pro5_{i}", width='stretch'):
                    ...
                    if copilot_ready:
                        # Quick questions should use FAQ cache, not context-aware LLM
        ...
        if user_q:
            ...
            if copilot_ready:
                # Free-form input also goes through FAQ first, then LLM fallback with Maverick system prompt
...
if hf["is_ready"] and hf["copilot"]:
    with st.sidebar:
        st.markdown("---")
        st.markdown('<div style="font-weight:700; color:#8b5cf6; font-size:0.9rem;">🤖 Maverick Copilot</div>', unsafe_allow_html=True)
...
<div style="text-align:center; font-size:0.7rem; color:#475569; padding:1rem 0;">
    Maverick-SORT Professional Edition v5.0 | Day 4 Commercialization Build | 2026/06/08<br>
