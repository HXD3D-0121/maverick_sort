...
### [Day 3] Scheduling Model Upgrade: Multi-Objective Optimization & Real-Time Adaptation

**Theme**: Evolving the scheduling model from "single-objective cost minimization" to "multi-objective Pareto optimal + real-time adaptive"

**Upgrade Content**:

**3.1 Multi-Objective Pareto Optimization**
- Current: Single reward function $r = r^{eff} + r^{comp} + r^{time} + r^{setup}$
- Upgrade: Explicitly maintain a **Pareto frontier**, allowing users (or the system) to select preferences by scenario:
...
- Implementation: NSGA-II or MOEA/D multi-objective evolutionary algorithms, combined with DRL

**3.2 Real-Time Adaptive Mechanisms**
- Dynamic arrival rate estimation: use Exponentially Weighted Moving Average (EWMA) for real-time order arrival prediction
- Dynamic wave capacity adjustment: during peak periods (arrival rate > 200% baseline), automatically reduce wave size and increase release frequency
- Online learning: after each shift completion, fine-tune the policy with new data (avoiding catastrophic forgetting)

**3.3 What-If Scenario Simulation Engine**
- Allow users to adjust parameters and see results in real time:
  - "If we add 2 cold-zone pickers, how much cost is reduced?"
  - "If we reduce wave capacity to 15 during peak periods, how does the delay rate change?"
- Implementation: Fast inference based on pre-trained model + parallel scenario evaluation

**3.4 Alternative Research Direction: BVN Matrix Decomposition (Literature Reference)**

The literature *Warehouse Assortment Selection with Constant-Factor Guarantees via Birkhoff-von Neumann Decomposition* has been uploaded to the project root. The core ideas of this method are highly relevant to our project:

- **BVN Theorem**: Any doubly stochastic matrix can be decomposed into a convex combination of permutation matrices, i.e., $D = \sum_k \lambda_k P_k$, where $\sum_k \lambda_k = 1$ and $P_k$ are permutation matrices
- **Mapping to Our Project**: Wave allocation can be viewed as an "order → wave" assignment matrix; ideally each order should be assigned to a unique wave (permutation matrix property)
- **Potential Optimization Direction**: Model wave allocation as a doubly stochastic matrix decomposition problem, leveraging BVN decomposition's constant-factor approximation guarantee to provide a **theoretical performance lower bound** for the DRL policy
- **Current Positioning**: As a **theoretical complement** to the Day 2 KGDRL main path, demonstrating in investor materials that "we have explored not only heuristics + DRL but also integration with classic operations research methods," showcasing academic depth

**Day 3 Deliverables**:
1. `multi_objective_scheduler.py` — Multi-objective scheduling engine
2. `what_if_simulator.py` — What-if scenario simulator
3. `adaptive_policy.py` — Real-time adaptive policy module
4. `bvn_research_note.md` — BVN matrix decomposition research notes (including mapping analysis to our project)
5. Update `AI_Tools_Usage_Review.md` — Day 3 timeline

**Investor Focus**:
> "Can the system handle unexpected situations?"
> → Real-time adaptation + What-if simulation = Enterprise Resilience

---

### [Day 4] Streamlit Commercial Adaptation: From "Course Assignment" to "Investor-Ready"

**Theme**: Comprehensive upgrade of Streamlit App's visual expression and business narrative capability

**4.1 New Commercial-Grade Page Modules (Eye-Catching for Investors)**

| New Page | Function Description | Investor Value | Technical Highlight Display |
|----------|---------------------|----------------|----------------------------|
| **💰 ROI Calculator** | Input enterprise parameters (daily orders, SKU count, labor cost), output annual savings estimate | Directly answers "how much money can be saved" | Real-time calculation engine, parameter slider linkage |
| **🏭 Competitor Radar** | Compare traditional WMS, SAP EWM, Manhattan Associates in functionality and cost | Demonstrates differentiation positioning | Dynamic radar chart + feature matrix heatmap |
| **📈 TCO Analysis** | 5-year Total Cost of Ownership comparison (traditional manual vs. this system) | Long-term value investment perspective | Line chart + sensitivity analysis dual view |
| **🔮 Scenario Lab** | Interactive What-if simulation (parameter sliders + real-time results) | Demonstrates system flexibility | Parallel multi-scenario rendering |
| **🤖 AI Insights** | Hugging Face LLM-generated natural language decision explanations | Lowers usage barriers | Real-time LLM streaming generation effect |
| **🧬 KGDRL Framework Viz** | Visualize the complete KGDRL workflow: Knowledge Graph → GAT Encoding → Policy Output | Technical depth visualization | Interactive graph network animation |
| **📊 Real-Time Digital Twin** | Warehouse digital twin panel: 2D warehouse layout + real-time wave flow animation + temperature zone heatmap | Immersive demonstration effect | Canvas animation + data-driven rendering |
| **🏆 Leaderboard** | Algorithm arena: PPO vs KGDRL vs TZU vs EDD and other 6 methods real-time PK | Algorithm superiority at a glance | Real-time leaderboard-style comparison |
| **🔔 Smart Alert Center** | Real-time panel for 6 types of anomaly alerts + root cause analysis + recommended actions | Demonstrates complete monitoring capability | Intelligent alert priority sorting |
| **📚 Patent & Research Wall** | Patent information display wall + core paper citations + technology roadmap | Academic endorsement visualization | Timeline-style patent journey |
